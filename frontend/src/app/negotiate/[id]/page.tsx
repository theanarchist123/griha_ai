...
export default function NegotiationPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000"}/api/negotiation/property/${propertyId}`);
  ...
  async function startBackendNeg() {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000"}/api/negotiation/start`, {
        method: "POST",
      ...
      if (json.status === "success") {
        setNegotiation(json.data);
        if (json.data?.id) {
          const sr = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000"}/api/negotiation/${json.data.id}`);
