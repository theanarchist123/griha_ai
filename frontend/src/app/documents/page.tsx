...
export default function DocumentsPage() {
  ...
  async function fetchDocuments() {
    setLoading(true);
    try {
      const res = await fetch(`${(process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000").replace(/\/$/, "")}/api/documents/`);
  ...
  async function uploadFile(file: File) {
    ...
    try {
      ...
      const res = await fetch(`${(process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000").replace(/\/$/, "")}/api/documents/upload`, {
        method: "POST",
        body: formData,
      });
      const json = await res.json();
  ...
  async function handleAskAI() {
    ...
    setAiAnswer("");
    try {
      const res = await fetch(`${(process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000").replace(/\/$/, "")}/api/documents/ask`, {
        method: "POST",
